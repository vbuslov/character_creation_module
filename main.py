...
def special(char_name: str, char_class: str) -> str:
    """Применение умений"""
    txt = 'применил специальное умение'
    if char_class == 'warrior':
        return (f'{char_name} {txt} «Выносливость {80 + 25}»')
    if char_class == 'mage':
        return (f'{char_name} {txt} «Атака {5 + 40}»')
    if char_class == 'healer':
        return (f'{char_name} {txt} «Защита {10 + 30}»')
    return (f'{char_name} не {txt}')


def start_training(char_name: str, char_class: str) -> str:
    """Проведение тренировки и бой с противником"""
    if char_class == 'warrior':
